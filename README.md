...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/tests-501-blue.svg" alt="501 tests">
  <img src="https://img.shields.io/badge/initial%20payload-81%20KB%20gzip-blue.svg" alt="81 KB gzip initial payload">
...
<p align="center">
  ...
  &nbsp;Â·&nbsp; free &nbsp;Â·&nbsp; open source &nbsp;Â·&nbsp; no accounts, no tracking
</p>
...
hard-codes a dozen model prices, and within a few months the entire premise is wrong â€” the models it
...
- **Estimate** â€” describe one interaction (paste your real prompt or move the sliders), set your scale,
  and see the monthly, yearly, per-user and margin numbers for up to four models side by side.
- **Compare** â€” every tracked model on a price-versus-capability value map, plus a sortable catalog you
  can select from directly, with the source and verification date for every row.
- **Learn** â€” seven short interactive lessons on tokens, why output costs more, compounding chat history,
  hidden reasoning tokens, caching and batching, and how the data pipeline works.
- **Data & Alerts** â€” pipeline health, full provenance for every number with a link to the vendor page it
...
Opt-in, and off unless a deployment is configured for them. Browser push stores nothing personal â€” a push
...
|                                    |                                                                                                                                                                                                 |
| ---------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Output is priced separately**    | Output typically costs 3â€“5Ã— input. Averaging the two, as many calculators do, understates most real workloads.                                                                               |
| **Chat history compounds**         | Turn _N_ re-sends turns 1â€¦*N*âˆ’1 as input, so conversation cost grows with the square of the turn count.                                                                                     |
| **Tokenizers differ per family**   | The same pasted text is counted with each model's own tokenizer â€” exactly (js-tiktoken, run in your browser) for OpenAI-family models and with a clearly labelled calibrated ratio elsewhere. |
| **Caching is not free**            | Cache _writes_ cost 1.25Ã— input at both OpenAI and Anthropic. Counting only the cheaper reads reports a saving your invoice will not have, so writes are billed and caching is off by default. |
| **Long context costs more**        | Above 272K input tokens OpenAI bills the whole request at 2Ã— input and 1.5Ã— output. Tiers apply per request, so a conversation can cross over partway through.                                |
...
| **Assumptions are visible**        | Every non-published number used in a calculation is listed under the results, not buried â€” and so is what the prices do not cover.                                                            |
...
```
  ...
  LiteLLM catalog â”€â”€â”¤                         â”‚
                    â”‚   allowlist â†’ merge â†’   â”‚â”€â”€â†’ public/data/pricing.json â”€â”€â†’ the app
  OpenRouter API â”€â”€â”€â”¤   validate â†’ diff       â”‚
                    â””â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”¬â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”˜
  data/pricing-overrides.json â”€â”€â”˜            â””â”€â”€â†’ docs/pricing-changelog.md
       (hand-verified, wins)
```

**The trust ladder**, in order:

1. **Hand-verified vendor rates** (`data/pricing-overrides.json`) â€” win every conflict.
2. **The LiteLLM community catalog** â€” the automated daily feed; thousands of models, updated within days
   of a release.
3. **The OpenRouter API** â€” an independent cross-check, never a source of record. Reseller pricing differs
   from first-party list pricing, so a disagreement of more than 20% _flags_ the model rather than
   changing it.
4. **Sanity rules** â€” schema validation, non-negative rates, implausibility checks, a hold on any price
...
A clean diff is committed and deployed automatically. A **newly** raised flag becomes a pull request â€” a
...
There is a free, keyless, CORS-open API at **[promptspend.dev](https://promptspend.dev)** â€” no account, no
...
```jsonc
{
  ...
  "providers": [{ "id": "openai", "name": "OpenAI", "country": "US", "pricingUrl": "https://â€¦" }],
  "models": [
    {
      "id": "gpt-5.6-terra",
      "providerId": "openai",
...
```bash
npm ci                 # `ci`, not `install` â€” this repo has a lockfile and CI honours it
...
```
src/lib/engine/     the cost engine â€” pure functions, no React, heavily tested
...
public/sw.js        service worker â€” push display only, no offline cache
...
Beyond the calculator, the build writes 159 crawlable pages â€” one per model, one per provider, and a
curated set of head-to-heads â€” from the same catalog and the same cost engine. See
...
colour-vision deficiency **in CI** â€” `src/lib/palette.test.ts` simulates protanopia and deuteranopia and
...
24Ã—24 (44Ã—44 on touch), no text renders below 12px, and there is no horizontal page scroll from 320px up.
Keyboard navigation throughout â€” including every point on the value map, which is a real button â€” focus is
...
| [docs/TESTING.md](docs/TESTING.md)                     | What the 501 tests cover, the uneven coverage thresholds, and what the suite deliberately does not cover  |
...
| [docs/API.md](docs/API.md)                             | The public pricing API on `promptspend.dev` â€” endpoints, why it fetches rather than bundles, going live |
...
| [docs/DEFERRED.md](docs/DEFERRED.md)                   | Work proposed and deliberately not done yet, with the reason â€” a decision, not a gap                    |
| [CONTRIBUTING.md](CONTRIBUTING.md)                     | Adding a model, the house style, and the rules that are not negotiable                                    |
| [SECURITY.md](SECURITY.md)                             | What is in scope â€” including a wrong price, which is treated as the most serious class of bug           |
...
**security@promptspend.com** or GitHub's private vulnerability reporting â€” see [SECURITY.md](SECURITY.md).
...
3. **No enabled control that does nothing.** Describe a planned feature â€” do not simulate it.
...
- **Scope of the prices.** Standard-tier, global-endpoint list prices in USD. Not modelled: regional and
  data-residency premiums (OpenAI and Anthropic both charge 1.1Ã—), fast/priority tiers, server-side tool
...
(a Cloudflare Worker â€” [docs/ALERTS.md](docs/ALERTS.md)). Precisely what changes:

- The Content Security Policy opens `connect-src` for that one API origin, and â€” only where Turnstile is
  configured â€” `script-src` and `frame-src` for `challenges.cloudflare.com`. Nothing else, ever. That
...
A deployment with no API configured â€” which is what this repository builds by default â€” keeps a strictly
...
MIT â€” see [LICENSE](LICENSE). The self-hosted typefaces (Space Grotesk, IBM Plex Sans, JetBrains Mono)
