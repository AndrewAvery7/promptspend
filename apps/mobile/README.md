...
- Data & Alerts shows pipeline health, public sync evidence, the trust ladder, flagged prices, commit-feed and repository options, privacy and support resources, and API/MCP/editor integrations. Its native Alert Center handles double opt-in, cadence, full-catalog model selection, passwordless preference management, and destructive unsubscribe without sending users to the website. Only Turnstile runs in an isolated WebView that receives no email address, model choices, scenario data, or prompt text. Native push remains withheld until the alerts Worker supports APNs/FCM tokens; browser VAPID subscriptions cannot be reused by native apps.
