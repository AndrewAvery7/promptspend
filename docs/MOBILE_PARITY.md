...
| Guided tour | `src/components/GuidedTour.tsx` | **Partial** — six-step accessible native walkthrough is implemented | Complete physical-device focus and largest-text checks |
...
| Light and dark interface | `src/state/useAppearance.ts` | **Partial** — persisted System, Light, and Dark choices are implemented | Complete physical-device dark-interface QA |
| Four accent colours | `src/state/useAppearance.ts`, `src/styles/tokens.css` | **Partial** — cobalt, emerald, teal, and violet share the website token pairs | Complete contrast and device QA for every combination |
...
| Share result | **Complete, adapted** — native share sheet | Retain readable native sharing |
...
| Email price alerts | **Adapted** — the app opens the live secure hosted Turnstile flow directly in Data & Alerts | A fully native form remains blocked on reviewed anti-abuse integration |
...
| Alert cadence, scope, followed models, and unsubscribe management | **Missing** | Preserve these outcomes in the native or secure hosted alert-management flow |
