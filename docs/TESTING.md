...
its own 137 tests. A total that is not re-derived from a run is a number that
...
| Suite                                          | Tests | Guards                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| ---------------------------------------------- | ----- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `scripts/lib/pipeline.test.ts`                 | 65    | The whole pipeline against fixtures: the trust ladder, both sanity thresholds, cold start, stale retention, flag-once semantics — including a standing flag whose _figures_ drift, which is what re-opened a pull request every morning — full-field diffing, coverage versus a price move versus a source correction, the content hash, changelog rendering, that a feed reporting the intro rate in force is not drift, and that the morning's vendor-page disagreement raises its flag without touching the price.                                                                                                                                                                                                                          |
| `scripts/lib/vendor-check.test.ts`             | 33    | The daily vendor-page check without the network: which rows can be checked (a provenance-only override against the catalog's figure), the blind extraction prompt, every outcome of comparing a page with the record — agreement within rounding, a moved standard or cached rate, a promotion the record lacks, a promotion withdrawn early, a page showing only the promotional figure — that only agreement moves a date, that a `verifyHint` reaches the reader without leaking the figure on record when one page prices the same model in several regional tables, that yesterday's report cannot raise today's flags, and the DeepSeek reader's JSON-mode request, its one retry, and every way a completion can carry no usable reply. |
...
| `packages/core/src/receipt/share.test.ts`      | 4     | The shared native/web parser accepts fenced and nested results, normalizes display strings, applies safe field bounds, preserves explicit defaults, and rejects malformed, incomplete, or oversized input.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| `packages/core/src/receipt/spec.test.ts`       | 2     | The shared instructions preserve the visible-message privacy boundary, current-price requirement, quality caveat, and byte-identical machine-readable contract.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| `packages/core/src/pricing/promo.test.ts`      | 2     | Promotional prices remain active through the last instant of their published end date and return to standard pricing immediately after expiration.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| `src/lib/pricing/catalog.test.ts`              | 40    | Schema validation including every malformed case the pipeline could produce, alias handling, the country filter and its counts, and the four states the chip can report.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
| `src/components/PromoRate.test.tsx`            | 8     | The shared rate marker: the promotional figure with its tooltip while a window is open, the standard figure and no marker once it closes or when there is no window, the last moment of the `until` day still inside it, and cache rates marked only when the vendor publishes a promotional one.                                                                                                                                                                                                                                                                                                                                                                                                                                              |
...
| `src/components/LaunchBanner.test.tsx`         | 7     | The launch announcement now that both apps are live: each official badge links to its own store, a QR code per store with the Android robot credited, a link to the permanent /app/ page, no form and no network request, a remembered dismissal, and that closing the old "coming soon" banner does not hide this one.                                                                                                                                                                                                                                                                                                                                                                                                                        |
...
| `src/lib/seo/render.test.ts`                   | 21    | Escaping a hostile display name out of both the markup and the JSON-LD, the policy hash matching what is emitted, no inline styles the policy would silently drop, that the footer offers the API these pages tell readers to prefer and the /app/ page, that the app page links both badges and QR codes to the live listings, and that a promotional rate is marked on the figure itself — during the window and not after — on every page that prints one.                                                                                                                                                                                                                                                                                  |
...
| `src/lib/seo/llms.test.ts`                     | 9     | That `llms.txt` stays an index rather than a data dump: absolute URLs, cheapest first, the boundary of what a price means, the catalog timestamp so staleness is visible, and that it names the MCP server, the extension and both native apps.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
...
| -------------- | ----- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `worker/`      | 102   | The alerts API inside workerd against a real D1 — including the RFC 8291 push crypto, checked byte-for-byte against the worked example in the RFC itself.                                                                                                                                                                                                                                                       |
