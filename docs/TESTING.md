...
**1091 tests in all**: 488 unit and integration tests in this package, 115 in
...
| `scripts/lib/vendor-check.test.ts`             | 26    | The daily vendor-page check without the network: which rows can be checked (a provenance-only override against the catalog's figure), the blind extraction prompt, every outcome of comparing a page with the record — agreement within rounding, a moved standard or cached rate, a promotion the record lacks, a promotion withdrawn early, a page showing only the promotional figure — that only agreement moves a date, and that yesterday's report cannot raise today's flags.                                  |
