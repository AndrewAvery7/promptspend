...
**1214 tests in all**: 502 unit and integration tests in this package, 224 in
...
| `scripts/lib/vendor-check.test.ts`             | 32    | The daily vendor-page check without the network: which rows can be checked (a provenance-only override against the catalog's figure), the blind extraction prompt, every outcome of comparing a page with the record — agreement within rounding, a moved standard or cached rate, a promotion the record lacks, a promotion withdrawn early, a page showing only the promotional figure — that only agreement moves a date, that yesterday's report cannot raise today's flags, and the DeepSeek reader's JSON-mode request, its one retry, and every way a completion can carry no usable reply. |
...
| `apps/mobile/` | 224   | The native launch product: all five tab outcomes plus PromptSpend Receipt, private prompt conversion, pricing-cache resilience, state recovery, sharing privacy, accessible country filtering, promotional-rate display, alert requests, guided-tour geometry, searchable Help & FAQs, startup routing, and the Turnstile message boundary.                                                                     |
